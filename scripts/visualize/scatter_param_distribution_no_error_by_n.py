...
fig, axs = plt.subplots(len(indexes), bins, figsize=(5.5 * bins, 5.5 * len(indexes)))
...
edge_left = [1, 11, 21, 31]
edge_right = [10, 20, 30, 40]
...
for idx, (left, right) in enumerate(
    ...
    axs[idx].set_title(
        r"\boldmath{$" + str(left) + r" \leq n \leq " + str(right) + r"$}",
        fontsize=20,
        fontweight="bold",
        pad=20,
    )
...
plt.savefig(
    os.path.join(
        model_ds_time_fig_directory, "parameter_scatter_distribution_n_re.png"
    ),
    bbox_inches="tight",
    pad_inches=0.25,
    format="png",
